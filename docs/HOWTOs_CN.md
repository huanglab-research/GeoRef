...
1. 准备训练数据集: [FFHQ](XXXX). 更多细节: [DatasetPreparation_CN.md](DatasetPreparation_CN.md#StyleGAN2)
...
1. 从 **ModelZoo** ([Google Drive](XXXXg), [百度网盘](XXXX)) 下载预训练模型到 `experiments/pretrained_models` 文件夹.
1. 测试.
...
1. 安装 [dlib](XXXX). 因为 DFDNet 使用 dlib 做人脸检测和关键点检测. [安装参考](XXXX).
    1. 克隆 dlib repo: `git clone git@github.com:davisking/dlib.git`
    1. `cd dlib`
    1. 安装: `python setup.py install`
2. 从 **ModelZoo** ([Google Drive](XXXX), [百度网盘](XXXX)) 下载预训练的 dlib 模型到 `experiments/pretrained_models/dlib` 文件夹.<br>
    你可以通过运行下面的命令下载 或 手动下载.

    > python scripts/download_pretrained_models.py dlib

3. 从 **ModelZoo** ([Google Drive](XXXX), [百度网盘](XXXX)) 下载 DFDNet 模型, 字典和人脸关键点模板到 `experiments/pretrained_models/DFDNet` 文件夹.<br>
     你可以通过运行下面的命令下载 或 手动下载.
