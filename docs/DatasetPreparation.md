...
1. Make [LMDB](XXXX), which could accelerate the IO and decompression speed during training.
1. [memcached](XXXX) is also supported, if they are installed (usually on clusters).
...
The implementation is to call the elegant fileclient design in [mmcv](XXXX). In order to be compatible with BasicSR, we have made some changes to the interface (mainly to adapt to LMDB). See [file_client.py](../basicsr/utils/file_client.py) for details.
...
1. `prefetch_mode: cuda`. Use CUDA prefetcher. Please see [NVIDIA/apex](XXXX#) for more details. It will occupy more GPU memory. Note that in the mode. you must also set `pin_memory=True`.
...
1. `prefetch_mode: cpu`. Use CPU prefetcher, please see [IgorSusmelj/pytorch-styleguide](XXXX#) for more details. (In my tests, this mode does not accelerate)
...
<table>
  ...
  <tr>
    ...
    <td rowspan="9"><a href="XXXX">Google Drive</a> / <a href="XXXX">Baidu Drive</a></td>
  </tr>
 <tr>
    <td><a href="XXXX">BSDS200</a></td>
    <td><sub>A subset (train) of BSD500 for training</sub></td>
  </tr>
  <tr>
    <td><a href="XXXX">General100</a></td>
  ...
  <tr>
    ...
    <td><a href="XXXX">DIV2K</a></td>
    <td><sub>proposed in <a href="XXXX">NTIRE17</a> (800 train and 100 validation)</sub></td>
    <td><a href="XXXX">official website</a></td>
  </tr>
 <tr>
    <td><a href="XXXX">Flickr2K</a></td>
 ...
 <tr>
  ...
  <tr>
    ...
    <td rowspan="2"><a href="XXXX">Google Drive</a> / <a href="XXXX">Baidu Drive</a></td>
  </tr>
 <tr>
    <td>OST300</td>
...
1. Download the datasets from the [official website](XXXX).
...
[Official webpage](XXXX)

1. Download the dataset: [`Septuplets dataset --> The original training + test set (82GB)`](XXXX).This is the Ground-Truth (GT). There is a `sep_trainlist.txt` file listing the training samples in the download zip file.
