...
1. 直接以图像/视频帧的格式存放在硬盘
2. 制作成 [LMDB](XXXX). 训练数据使用这种形式, 一般会加快读取速度.
3. 若是支持 [Memcached](XXXX), 则可以使用. 它们一般应用在集群上.
...
实现是调用了[MMCV](XXXX) 优雅的 FileClient 设计. 为了兼容 BasicSR, 我们对接口做了一些改动 (主要是为了适应LMDB), 参见 [file_client.py](../basicsr/utils/file_client.py).
...
1. `prefetch_mode: cuda`. 使用 CUDA prefetcher, 具体介绍参见 [NVIDIA/apex](XXXX#). 它会多占用一些GPU显存. 注意: 这个模式下, 一定要设置 `pin_memory=True`
...
1. `prefetch_mode: cpu`. 使用 CPU prefetcher, 具体介绍参见 [IgorSusmelj/pytorch-styleguide](XXXX#). (目前测试，这个加速不明显)
...
1. 下载数据: [`Septuplets dataset --> The original training + test set (82GB)`](XXXX). 这些是Ground-Truth. 里面有`sep_trainlist.txt`文件来区分训练数据.
1. 生成低分辨率图片. (TODO)
...
1. 下载 FFHQ 数据集. 推荐从 [NVlabs/ffhq-dataset](XXXX) 下载 tfrecords 文件.
