...
from .arch_util import ResidualBlockNoBN, ResidualBlockNoESA, default_init_weights, make_layer, srntt_init_weights, ResidualDDFBlock
...
class ContentExtractor(nn.Module):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, n_blocks=16):
        ...
        self.body = make_layer(
            ResidualBlockNoESA, n_blocks, nf=nf)
...
class C2MatchingNet(nn.Module):

    def __init__(self, ngf=64, n_blocks=16, groups=8):
        super(C2MatchingNet, self).__init__()
    ...
    def forward(self, x, pre_offset, img_ref_feat):
        ...
        upscale_restore, middle_feat = self.dyn_agg_restore(content_feat, pre_offset,
                                               img_ref_feat)
        return upscale_restore + base, middle_feat
...
class DynamicAggregationRestoration(nn.Module):
    ...
    def __init__(self, ngf=64, n_blocks=16, groups=8):
        ...
        self.body_small = make_layer(
            ResidualBlockNoESA, n_blocks, nf=ngf)
