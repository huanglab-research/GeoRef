...
class MultiRefRestorationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.net_mask = build_network(opt['network_mask']).cuda()
        ...
        # define network for vgg
        self.net_vgg = build_network(opt['network_vgg'])
        self.net_vgg = self.model_to_device(self.net_vgg)
        ...
        if load_path is not None:
            self.load_net_g_network(self.net_g, load_path,
                              self.opt['path']['strict_load'])
        if self.is_train:
            self.net_g.train()
    ...
    def optimize_parameters(self, step):
        ...
        self.base_feat, self.sisr = self.net_rrdb(self.img_in_lq)
        ...
        self.output = self.net_g(self.sisr, self.base_feat, self.pre_corr, self.corr_mask, self.img_ref_feat)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            img_name = osp.splitext(osp.basename(val_data['in_path'][0]))[0]
